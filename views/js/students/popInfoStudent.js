...
var openPopInfoStudent;

(function($){

	var container  = $("#popInfoStudent");
	 ...
	 * ABRE O POP UP
	 */
	openPopInfoStudent = function (itemId){			
		clearPopInfoStudent();

		container.modal('show'); 
		loadInfoStudent(itemId);	
	}
	 ...
	 * CARREGA OS DADOS
	 */
	function loadInfoStudent(itemId){
		...
		$.ajax({
			...
			url 	: _FULLPATH +'/controllers/Students.php',
			dataType: 'json',				
	 ...
	 * FECHA O POP UP
	 */
	function closePopInfoStudent(){		
		clearPopInfoStudent();
		$("#popInfoStudent").modal('hide');		
	};	
	...
	/** INIT */
	$(function(){
		container.find('#closeInfoStudentBtn, #cancelInfoStudentBtn').click(closePopInfoStudent);
	});
